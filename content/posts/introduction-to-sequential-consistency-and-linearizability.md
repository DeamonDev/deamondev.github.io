...
\(\operatorname{SeqSpec}(X)\) which is a set of legal sequential histories over a given object (all of these adjectives
are formally defined in the very next section) . By *history* we mean
**sequence** of *events*.
...
need to order events (old events from \(\Sigma\) and new formal points) in \(\widetilde{\Sigma}\) with the constraints
being the aforementioned inequalities on formal points.
